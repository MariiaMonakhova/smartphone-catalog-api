...
return [
    ...
    'info' => [
         ...
         */
        'description' => <<<'MD'
            REST API for managing a catalog of smartphones.

            **Prices are stored in USD.** Add `?currency=USD|EUR|UAH` to the list and
            single-product endpoints to receive the converted `price` alongside the
            original `price_usd`. Rates come from the National Bank of Ukraine and are
            cached daily. If a rate is needed but unavailable, conversion endpoints
            respond with **503 Service Unavailable** (USD requests never fail).

            Use the **Try It** panel on any endpoint to send live requests against your
            local server.
            MD,
    ],

    'ui' => [
        'title' => 'Smartphone Catalog API',
    ],
